...
export default function AdminHome() {
  ...
  // Fungsi untuk mengecek visibilitas satu section
  ...
  // Fungsi untuk mengecek semua section
  const checkAllSectionsVisibility = () => {
    const sections = ['about', 'features', 'statistics', 'services'];
    sections.forEach(checkSectionVisibility);
  };

  // Animasi scroll - refactored
  useEffect(() => {
    window.addEventListener('scroll', checkAllSectionsVisibility);
    checkAllSectionsVisibility(); // Panggil sekali saat mount
    
    return () => window.removeEventListener('scroll', checkAllSectionsVisibility);
  }, []);

  const stats = [
    { id: "skema", number: statsData.totalSkema.toString(), label: "Skema Sertifikasi", icon: "📋" },
    { id: "asesor", number: statsData.totalAsesor.toString(), label: "Asesor Kompeten", icon: "👨‍🏫" },
    { id: "tuk", number: statsData.totalTUK.toString(), label: "Tempat Uji Kompetensi (TUK)", icon: "🏫" },
  ];
    ...
    <>
      ...
      <section className={styles.ctaSection}>
        <div className={styles.ctaContent}>
          ...
          <p>Dapatkan sertifikasi kompetensi yang diakui secara nasional</p>
          <Link href="/sertifikasi/pendaftaran-sertifikasi" className={styles.btnPrimary}>
            Daftar Sekarang
          </Link>
        </div>
      </section>
