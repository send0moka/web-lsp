...
export default function HasilSertifikasi() {
  ...
  const filteredHasil = hasil.filter((item) => {
    ...
    const matchSkema = filterSkema.length > 0
      ? filterSkema.some(s => s.toLowerCase() === item.nama_skema?.toLowerCase())
      : true;

    const matchPlant = filterPlant.length > 0
      ? filterPlant.some(p => p.toLowerCase() === item.plant?.toLowerCase())
      : true;

    const matchSeksi = filterSeksi.length > 0
      ? filterSeksi.some(s => s.toLowerCase() === item.seksi?.toLowerCase())
      : true;

    const matchPerusahaan = filterPerusahaan.length > 0
      ? filterPerusahaan.some(p => p.toLowerCase() === item.company?.toLowerCase())
      : true;
  ...
  // Helper function untuk render kolom hasil
  const renderHasil = (item) => {
    if (!item.hasil) {
      return (
        <select
          defaultValue=""
          onChange={(e) => handleUpdateHasil(item.id_pendaftaran, e.target.value)}
          className={styles.selectHasil}
        >
          ...
          <div className={styles.filterRow}>
            ...
            <div className={styles.filterDropdown} ref={perusahaanRef}>
              ...
              {showPerusahaanFilter && (
                <div className={styles.filterMenu}>
                  {uniquePerusahaan.map((perusahaan) => (
                    <label key={perusahaan} className={styles.checkboxItem}>
                      <input
                        type="checkbox"
                        checked={filterPerusahaan.includes(perusahaan)}
                        onChange={() => togglePerusahaan(perusahaan)}
                      />
                      {perusahaan || "-"}
                    </label>
                  ))}
    ...
    <>
      ...
      <section className={styles.hero}>
        <div className={styles.heroContent}>
          <h1 className={styles.heroTitle}>Hasil Sertifikasi</h1>
